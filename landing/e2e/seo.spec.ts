...
test('homepage publishes complete social preview metadata', async ({ request }) => {
  ...
  expect(html).toContain('property="og:site_name" content="SmoothScroll"')
...
test('homepage renders total release downloads in stats and download buttons', async ({ page }) => {
  await page.route('https://api.github.com/repos/quangtruong2003/SmoothScroll/releases?per_page=100', async (route) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify([
        { tag_name: 'v1.0.0', assets: [{ name: 'setup.exe', browser_download_url: '', download_count: 12 }] },
        { tag_name: 'v1.1.0', assets: [{ name: 'setup.exe', browser_download_url: '', download_count: 34 }] },
      ]),
    })
  })

  await page.goto('/')
  await expect(page.locator('[data-stat="Downloads"]')).toContainText('46')
  await expect(page.getByTestId('download-cta-count').first()).toContainText('46')
})

test('homepage publishes a consistent preferred site name', async ({ request }) => {
  const html = await (await request.get('/')).text()
  const match = html.match(/<script type="application\/ld\+json">(.*?)<\/script>/)

  expect(html).toContain('name="application-name" content="SmoothScroll"')
...
test('homepage targets high-opportunity Windows and download queries in visible copy', async ({ request }) => {
  const html = await (await request.get('/')).text()

  expect(html).toContain('Free smooth scrolling software for Windows 10 and 11.')
  expect(html).toContain('Download SmoothScroll for Windows')
  expect(html).toContain('Is SmoothScroll safe?')
})

test('English guide targets informational smooth-scrolling intent', async ({ request }) => {
  const html = await (await request.get('/how-it-works/')).text()

  expect(html).toContain('<title>How SmoothScroll Works: Windows Scroll Settings Guide | Mouse Wheel Smoothing</title>')
  expect(html).toContain('See how SmoothScroll')
  expect(html).toContain('What smooth scrolling does')
})
